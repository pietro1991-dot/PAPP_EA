...
// --- Esecuzione: entrata a LIMITE (lascia venire il prezzo) + anti-churn ---
input bool   UseLimit      = false; // false = MARKET (validato). true = LIMITE: SCONSIGLIATO su reversione (adverse selection: il limite si riempie solo quando il prezzo CONTINUA contro la reversione).
input double LimitOffsetPip = 3.0;  // pip di scostamento del limite (buy sotto / sell sopra). 0 = a bid/ask
input int    LimitExpiryBars= 1;    // barre di validita' del limite non riempito (poi si annulla e ri-valuta)
input int    CooldownBars   = 3;    // barre di pausa dopo un'uscita (anti-churn). 0 = off

// --- Diagnostica (solo tester): stampa osc + conta i trade per confronto col CSV ---
input bool   DebugDump     = true;  // true = stampa nel Journal i primi bar (osc) e i totali a fine test
...
int      g_cooldownLeft = 0;   // barre di pausa rimanenti dopo un'uscita
int      g_pendAge = 0;        // barre da quando il limite e' pendente (per la scadenza)
int      g_dbgN = 0;           // quante righe di dump gia' stampate
int      g_nBuy = 0, g_nSell = 0;  // conteggio entrate (diagnostica)
...
void OnDeinit(const int reason)
  {
   if(DebugDump && g_tester)
      PrintFormat("DBG TOTALI: entrate BUY=%d  SELL=%d  (Python config validata attende ~40-50/anno)", g_nBuy, g_nSell);
...
//+------------------------------------------------------------------+
// Ordini pendenti (limite) del nostro EA
bool HasPending()
  {
...
void OnTick()
  {
   ...
   // --- DIAGNOSTICA: dump osc dei primi bar validi (confronto col CSV) ---
   if(DebugDump && g_tester && g_dbgN<60)
     {
      double dbgMed=0; ReadMedianD1(iBarShift(_Symbol,PERIOD_D1,iTime(_Symbol,ANCHOR_TF,1),false)+1,dbgMed);
      PrintFormat("DBG %s close1=%.5f med=%.5f osc=%.2f",
                  TimeToString(iTime(_Symbol,ANCHOR_TF,1),TIME_DATE|TIME_MINUTES),
                  iClose(_Symbol,ANCHOR_TF,1), dbgMed, osc);
      g_dbgN++;
     }
   ...
   // 1) POSIZIONE APERTA -> gestisci uscita (held dal tempo REALE d'apertura)
   long dir=0;
   if(HasPosition(dir))
     {
      ...
      int  held = iBarShift(_Symbol,ANCHOR_TF,posTime,false);
      bool timeOut = (MaxHoldBars>0 && held>=MaxHoldBars);
      if(revertDone || timeOut){
         if(trade.PositionClose(_Symbol)){
            g_cooldownLeft = CooldownBars;                          // anti-churn dopo l'uscita
            if(!g_tester) PappSignal("close",_Symbol,(int)(dir>0?1:2),0,0,0,0,(dir>0?bid:ask),move,timeOut?"uscita max-hold":"uscita osc=50");
           }
        }
      return;
     }

   // 2) ordine LIMITE pendente: aspetta il fill, o annulla se scaduto / segnale sparito
   if(HasPending())
     {
      g_pendAge++;
      if(g_pendAge>=LimitExpiryBars || !(osc<LoThr || osc>HiThr)){ CancelMyPending(); g_pendAge=0; }
      else return;
     }

   if(!g_licensed) return;
   if(g_cooldownLeft>0){ g_cooldownLeft--; return; }               // pausa anti-churn

   // 3) nuova entrata (LIMITE = lascia venire il prezzo, meno spread; oppure market)
   double lot=CalcLot();
   if(lot<=0.0) return;

   if(osc < LoThr)
     {
      double entryP = UseLimit ? (bid - LimitOffsetPip*pip) : ask;
      double sl = (SafetySLpip>0.0)? entryP - SafetySLpip*pip : 0.0;
      double tp = (TPlongPip >0.0)? entryP + TPlongPip *pip : 0.0;
      bool ok = UseLimit ? trade.BuyLimit(lot,entryP,_Symbol,sl,tp,ORDER_TIME_GTC,0,"reversione buy-limit")
                         : trade.Buy(lot,_Symbol,ask,sl,tp,"reversione buy");
      if(ok){ g_pendAge=0; g_nBuy++; if(!g_tester) PappSignal("open",_Symbol,1,entryP,sl,tp,lot,0,0,StringFormat("BUY osc=%.0f<%.0f",osc,LoThr)); }
     }
   else if(osc > HiThr)
     {
      double entryP = UseLimit ? (ask + LimitOffsetPip*pip) : bid;
      double sl = (SafetySLpip>0.0)? entryP + SafetySLpip*pip : 0.0;
      double tp = (TPshortPip>0.0)? entryP - TPshortPip*pip : 0.0;
      bool ok = UseLimit ? trade.SellLimit(lot,entryP,_Symbol,sl,tp,ORDER_TIME_GTC,0,"reversione sell-limit")
                         : trade.Sell(lot,_Symbol,bid,sl,tp,"reversione sell");
      if(ok){ g_pendAge=0; g_nSell++; if(!g_tester) PappSignal("open",_Symbol,2,entryP,sl,tp,lot,0,0,StringFormat("SELL osc=%.0f>%.0f",osc,HiThr)); }
     }
  }
//+------------------------------------------------------------------+
