...
int OnInit()
  {
   ...
   for(int i=0;i<7;i++)
     {
      hMA[i] = iMA(_Symbol,ANCHOR_TF,TimeToBars(gDays[i]),0,MODE_SMA,PRICE_CLOSE);
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   if(rates_total<2) { Print(__FUNCTION__,": rates_total<2"); Alert("PaPP Median: insufficient data"); return 0; }
...
void DrawInfo()
  {
   ...
   string sMA[7];
   for(int m=0;m<7;m++) sMA[m]=StringFormat("MA %3dg: %s",gDays[m],DoubleToString(g_cache.maVal[m],_Digits));
