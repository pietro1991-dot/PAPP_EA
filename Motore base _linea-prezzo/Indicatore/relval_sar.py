...
def cross_series():
    if a.tf=="D1":
        ...
        df=df.resample(_RULE[a.tf]).agg(EUR=("EUR","last"),GBP=("GBP","last")).dropna()
    ...
    while i<n-1:
        ...
        for k in range(i+1,min(i+1+a.hold,n)):
            ...
            if (not reversed_once) and sar_on and move<=-a.sar:
                # chiudi reversione in perdita, INVERTI (cavalca trend)
                trades.append(-a.sar - a.cost); tdate.append(d_idx_time[di])
...
print(f"\n=== EURGBP relval STOP-AND-REVERSE | TF={a.tf} sar={a.sar}pip cost={a.cost} ma={a.mawin} hold={a.hold} ===")
